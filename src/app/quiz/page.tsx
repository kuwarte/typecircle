...
const QUIZ_PROGRESS_KEY = "typecircle:quiz-progress";
...
function saveProgress(progress: SavedProgress) {
    ...
    try {
        ...
        // sessionStorage unavailable — non-critical, progress just won't resume
    }
}
...
export default function QuizPage() {
    ...
    // Persist progress (including question order) while on the quiz step
    useEffect(() => {
        if (step !== "quiz") return;
        saveProgress({
            qIndex,
                    ...
                    <div className="flex flex-col justify-between h-full gap-10 max-w-2xl mx-auto w-full">
                        ...
                        <div className="max-w-md mx-auto w-full">
                            <div
                                ...
                                role="radiogroup"
                                aria-label="How much does this describe you?"
                            >
                                {SCALE.map((s) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            >
                                                {isSelected ? <Check size={13} strokeWidth={3} /> : s.value}
                                            </span>
                                        </button>
                                    );
                                })}
                        ...
                        <div className="bg-[var(--color-ink)]/[0.02] border border-[var(--color-ink)]/10 rounded-3xl p-8 md:p-14 shadow-sm">
                            <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-12 items-stretch">
                                ...
                                <div className="flex flex-col justify-between h-full">
                                    <div>
                                        ...
                                        {result.isClose && result.secondaryType && (
                                            <div className="mt-4 flex items-start gap-2 rounded-xl bg-[var(--color-accent)]/8 text-[var(--color-accent-ink)] p-3 text-xs font-medium border border-[var(--color-accent)]/20">
                                                <AlertCircle size={14} strokeWidth={2.5} className="shrink-0 mt-0.5" />
                                                <p>
                                                    Your answers were close between Type {result.primary_type} and Type{" "}
                                                    {result.secondaryType}. We picked {result.primary_type} by a narrow
                                                    margin — it's worth reading{" "}
                                                    <Link
                                                        href={`/types/${result.secondaryType}`}
                                                        className="underline hover:text-[var(--color-accent)] transition-colors"
                                                    >
                                                        Type {result.secondaryType}
                                                    </Link>{" "}
                                                    too before settling on one.
                                                </p>
                                            </div>
                                        )}
                                ...
                                <div className="bg-[var(--color-paper)] border border-[var(--color-ink)]/10 rounded-2xl p-6 md:p-8 shadow-sm">
                                    ...
                                    <p className="text-xs text-[var(--color-ink)]/40 leading-relaxed -mt-2 mb-6">
                                        Based on {QUIZ_QUESTIONS.length} statements rated on a 1–5 scale,
                                        normalized against your own average rating to reduce rating-tendency
                                        bias.{" "}
                                        <Link href="/about#scoring" className="underline hover:text-[var(--color-accent)] transition-colors">
                                    ...
                                    {chatMessages.length === 0 && (
                                        <div className="text-center py-6 px-4">
                                            <p className="text-sm font-medium text-[var(--color-ink)]/50">
                                                ...
                                                relationships, under stress, whatever you're curious about.
                                            </p>
                                        </div>
                                    )}
                                ...
                                >
                                    ...
                                    >
                                        {chatLoading ? (
                                            ...
                                            <Send size={15} strokeWidth={2.5} className="mr-0.5 mt-0.5" />
                                        )}
                                    </button>
                                </form>
                            </div>
