...
export default function GroupChatPage({
    ...
    return (
        <section className="relative max-w-3xl mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 flex flex-col h-[calc(100dvh-4rem)] overflow-hidden">
            <style jsx global>{`
        .chat-scroll {
      ...
      `}</style>
            ...
            {/* Header — tighter padding on mobile so it doesn't eat into the
          already-constrained viewport height */}
            <div className="flex items-center gap-2.5 sm:gap-3 pb-3 sm:pb-5 border-b border-[var(--color-ink)]/10 shrink-0">
                <Link
                    href={`/community/${groupId}`}
                    className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full border border-[var(--color-ink)]/10 text-[var(--color-ink)]/60 hover:border-[var(--color-accent)]/40 hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/5 transition-colors shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/50"
                >
                    <ArrowLeft size={16} strokeWidth={2} className="sm:hidden" />
                    <ArrowLeft size={17} strokeWidth={2} className="hidden sm:block" />
                </Link>
                <div className="min-w-0 flex-1">
                    <h1 className="font-heading font-semibold text-base sm:text-xl tracking-tight leading-tight truncate">
                        {groupName ?? "Circle chat"}
                    </h1>
                    <p className="flex items-center gap-1.5 text-[11px] sm:text-xs text-[var(--color-ink)]/40 mt-0.5">
            ...
            <div className="relative flex-1 min-h-0">
                ...
                <div
                    ...
                    className="chat-scroll absolute inset-0 overflow-y-auto flex flex-col py-3 sm:py-5"
                >
                    {loadingOlder && (
                    ...
                    {loadingChat && (
                        <div className="flex-1 flex flex-col gap-5">
                            {Array.from({ length: 6 }).map((_, i) => (
                                <div
                                    key={i}
                                    className={`flex flex-col gap-2 ${i % 3 === 0 ? "items-end" : "items-start"
                                        }`}
                                >
                    ...
                    {!loadingChat &&
                        ...
                        messages.map((msg, i) => {
                            ...
                            const reactionCounts = msg.reactions.reduce<Record<string, number>>(
                                (acc, reaction) => {
                            ...
                            return (
                                <div key={msg.id} className="msg-in">
                                    ...
                                    <div
                                        className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : ""} ${sameSenderAsPrev ? "mt-0.5" : "mt-2.5 sm:mt-3"
                                            }`}
                                    >
                                        {!isMe && (
                                            <Avatar
                                                className={`w-6 h-6 shrink-0 rounded-lg border border-[var(--color-ink)]/10 mb-0.5 ${sameSenderAsPrev ? "opacity-0" : ""
                                                    }`}
                                            >
                                                <AvatarImage
                                        ...
                                        <div
                                            className={`group relative max-w-[82%] sm:max-w-[75%] flex flex-col gap-1 ${isMe ? "items-end" : "items-start"
                                                }`}
                                        >
                                            ...
                                            {/* Action row: on touch devices there's no hover, so
                          this stays reachable via tap-to-reveal rather than
                          relying purely on group-hover. It's kept visible
                          via opacity/scale that also responds to
                          group-active for a tap. */}
                                            <div
                                                className={`absolute -top-9 ${isMe ? "right-0" : "left-0"} z-10 flex items-center gap-0.5 rounded-full border border-[var(--color-ink)]/10 bg-[var(--color-paper)] px-1 py-1 opacity-0 shadow-md scale-95 origin-bottom transition-all duration-150 group-hover:opacity-100 group-hover:scale-100 group-focus-within:opacity-100 group-focus-within:scale-100`}
                                            >
                                                <button
                                                    onClick={() => handleAddReaction(msg.id)}
                                                    className={`p-1.5 rounded-full hover:bg-[var(--color-ink)]/5 transition-colors ${iReacted
                                                ...
                                                <div className="relative">
                                                    ...
                                                    {openMenuId === msg.id && (
                                                        <div
                                                            ...
                                                            className={`absolute top-full mt-1 ${isMe ? "right-0" : "left-0"
                                            ...
                                            {msg.replyTo && (
                                                <div
                                                    className={`flex items-stretch gap-2 max-w-full px-1 ${isMe ? "flex-row-reverse text-right" : ""
                                                        }`}
                                                >
                                            ...
                                            <div className="flex items-end gap-1.5">
                                                <div
                                                    ...
                                                    className={`px-3.5 py-2 rounded-2xl text-[14px] leading-relaxed cursor-default select-text shadow-sm transition-shadow hover:shadow-md break-words ${isMe
                                                ...
                                                <span className="text-[10px] text-[var(--color-ink)]/30 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity shrink-0 pb-1 tabular-nums">
                                                    ...
                                                    {Object.entries(reactionCounts).map(
                                                        ([emoji, count]) => (
                                                            <button
                                                                ...
                                                                onClick={() => handleAddReaction(msg.id)}
                                                                type="button"
                                                                className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] transition-all ${emoji === HEART_REACTION && iReacted
                                                            ...
                                                            >
                                                                {emoji === HEART_REACTION ? (
                                                                    <Heart
                                                                        size={10}
                                                                        strokeWidth={2.25}
                                                                        className={`text-[var(--color-accent)] ${poppingId === msg.id ? "heart-pop" : ""
                                                                            }`}
                                                                        fill="currentColor"
                                                                    />
                                                                ) : (
            ...
            {/* Input — pb includes safe-area inset so it clears the home
          indicator/gesture bar on notched phones instead of sitting flush
          against the very bottom edge */}
            <div
                className="shrink-0 border-t border-[var(--color-ink)]/10 pt-3 sm:pt-4"
                style={{ paddingBottom: "max(env(safe-area-inset-bottom), 0px)" }}
            >
                {(replyTo || typingFeedback) && (
                    <div className="mb-2.5 border-l-3 border-[var(--color-accent)]/50 bg-[var(--color-ink)]/[0.04] px-3 sm:px-4 py-2.5 text-sm text-[var(--color-ink)]">
                        {replyTo ? (
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <input
                        ...
                        // zoom-on-focus for inputs under 16px; drops to text-sm on
                        // larger screens where that's not an issue.
                        className="flex-1 min-w-0 rounded-full border border-[var(--color-ink)]/12 bg-[var(--color-paper)] px-4 py-2.5 text-base sm:text-sm outline-none transition-colors focus:border-[var(--color-accent)] focus:ring-4 focus:ring-[var(--color-accent)]/10 disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <button
                        onClick={handleSend}
                        disabled={!canSend}
